...
const TransactionInterface = () => {
  ...
  const { config: configWrite, error } = usePrepareContractWrite({
    ...
    args: [ethers.utils.parseEther("0.1"), lockInEnd, limit, false, "EcoSeeds Token 1", "EST1"], 
    suspense: true,
  });
  ...
  return (
    ...
    <>
      ...
      {error && (
       ...
       {isSuccess && <div>Transaction: <Link target='_blank' href={`https://alfajores.celoscan.io/tx/${JSON.stringify(data?.hash).slice(1, -1)}`}>{JSON.stringify(data?.hash)}</Link></div>}
